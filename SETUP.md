...
Choose Claude Code or Codex. Both use the same profiles, workflows, and portal CLIs.

### Codex

Install or open Codex using the [official setup guide](https://developers.openai.com/codex/quickstart/).
Start it in this repository. `AGENTS.md` supplies project instructions and
`.agents/skills/` supplies the workflow skills. Use `$job-profile`, `$job-setup`,
`$job-scraper`, and `$job-apply` in the conversation; see [docs/CODEX.md](docs/CODEX.md)
for all commands. No Claude Code or Gemini installation is required for Codex.
...
The job portal CLIs (LinkedIn, Indeed UK, Adzuna, company ATS boards, jobs.ac.uk, RemoteOK, WeWorkRemotely) are written in TypeScript and run with Bun.
...
For Codex, the same Poppler package also provides `pdfinfo` and `pdftoppm` for
page counts and rendering PDFs to inspect visually. Install it unless your client
already provides a PDF renderer; visual verification is required for finished applications.
...
```powershell
$tools = @("linkedin-search", "indeed-search", "adzuna-search", "ats-search", "jobsacuk-search", "remoteok-search", "weworkremotely-search")
...
- Bash / zsh / Git Bash:
```bash
for tool in linkedin-search indeed-search adzuna-search ats-search jobsacuk-search remoteok-search weworkremotely-search; do
...
If you're outside the UK, you can generate an equivalent search skill for your local job board with `/add-portal` — it scaffolds the same CLI structure for any public portal and test-runs a live query before registering. See the "Job search tools" section in the README.

## 4. Run the setup interview

In Codex, start `codex` from the repository root, select the person with
`$job-profile <slug>` (or create one with `$job-profile --new <slug>`), then run
`$job-setup` in the conversation. Real profiles remain local and ignored by Git. The steps below
use Claude Code's command names; the same onboarding paths apply to Codex.
...
Create a local profile from the blank scaffold, then run onboarding:
...
Use your own slug. Do not populate the tracked `profiles/example/` template.

Claude will offer three paths:

- **Path A (documents folder):** Add your CV, LinkedIn export, diplomas, references, or past applications under `profiles/<slug>/documents/`. Claude reads and cross-references them before proposing profile updates. This is best when you have several source files.
...
| File | Content |
|------|---------|
| `profiles/<slug>/profile/00-summary.md` | Your full candidate profile |
...
| `profiles/<slug>/profile/03-cv-statements.md` | Profile statement templates for your background |
| `07-interview-prep.md` | STAR examples from your experience |
| `profiles/<slug>/cv/main_example.tex` | Your LaTeX CV with actual details |
| `profiles/<slug>/profile/search-queries.md` | Job search queries for `/scrape` |
...
In Codex, use `$job-apply` in place of `/apply` in the examples below.
...
The application workflow compiles and inspects both PDFs before delivery. To
recompile manually later, replace `<slug>`, `<company>`, and `<role>` with actual
values and start from the repository root:

```bash
# Bash / zsh / Git Bash
(cd "profiles/<slug>/cv" && lualatex -interaction=nonstopmode "main_<company>.tex")
(cd "profiles/<slug>/cover_letters" && xelatex -interaction=nonstopmode "cover_<company>_<role>.tex")
```

```powershell
# PowerShell
Push-Location "profiles/<slug>/cv"
lualatex -interaction=nonstopmode "main_<company>.tex"
Pop-Location
Push-Location "profiles/<slug>/cover_letters"
...
The cover letter template expects fonts in `profiles/<slug>/cover_letters/OpenFonts/fonts/`. Make sure this directory exists and contains the Lato and Raleway font files.
