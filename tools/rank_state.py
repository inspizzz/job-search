...
Usage:
  python3 tools/rank_state.py candidates --profile <slug> [--all] [--focus TEXT] [--limit N]
  python3 tools/rank_state.py sweep --profile <slug> [--write] [--exclude KEY,KEY]
  python3 tools/rank_state.py apply --profile <slug> --results results.json [--dry-run]
...
def load_state(path: Path) -> tuple[dict, dict]:
    ...
    if any(not isinstance(entry, dict) for entry in seen.values()):
        sys.exit(f"{path}: every job entry must be an object")
    return doc, seen
...
def cmd_apply(args) -> int:
    ...
    for result in results:
        try:
            ...
            if not isinstance(key, str) or key not in seen:
                raise ValueError("no such key in seen_jobs.json")
            if key in keys:
                raise ValueError("duplicate result key")
            keys.add(key)
            status = result.get("status")
            if status not in {"scored", "expired", "retrieval_failed"}:
                raise ValueError("status must be scored, expired, or retrieval_failed")
            if status == "scored":
                scores = result.get("scores")
                if not isinstance(scores, dict):
                    raise ValueError("scores must be an object")
                overall_score(scores)
                for field in ("location_verdict", "language_gate", "eligibility_gate"):
                    if field in result and result[field] not in {"PASS", "FAIL", "FLAG"}:
                        raise ValueError(f"invalid {field}")
                for field in ("strengths", "gaps"):
                    if field in result and (not isinstance(result[field], list) or
                                           any(not isinstance(v, str) for v in result[field])):
                        raise ValueError(f"{field} must be an array of strings")
                if result.get("deadline") and parse_iso(result["deadline"]) is None:
                    raise ValueError("deadline must be an ISO date")
        except (ValueError, TypeError) as exc:
            errors.append({"key": result.get("key") if isinstance(result, dict) else None, "error": str(exc)})
    if errors:
        print(json.dumps({"ranked": [], "vetoed": [], "expired": [], "errors": errors, "written": False}))
        return 1
    ...
    for result in results:
        ...
        if result.get("status") == "retrieval_failed":
            # Keep the entry eligible for a later retry. Retrieval failure is not expiry.
            continue
        ...
        entry["location_verdict"] = result.get("location_verdict") or legacy or "FLAG"
        entry["language_gate"] = result.get("language_gate") or "FLAG"
        entry["eligibility_gate"] = result.get("eligibility_gate") or "FLAG"
        entry["eligibility_note"] = result.get("eligibility_note")
    ...
    veto = lambda r: any(r[field] == "FAIL" for field in ("location_verdict", "language_gate", "eligibility_gate"))
...
def main() -> int:
    ...
    common.add_argument("--profile", required=True, help="explicit ignored local profile slug")
    common.add_argument("--state", type=Path, help="optional state path within that profile")
    ...
    cand.add_argument("--tracker", type=Path, help="optional tracker path within that profile")
    ...
    try:
        ...
        args.state = profile_path(profile, args.state or "job_scraper/seen_jobs.json")
        args.tracker = profile_path(profile, getattr(args, "tracker", None) or "job_search_tracker.csv")
        if hasattr(args, "results"):
            args.results = profile_path(profile, args.results)
