...
**Output file:** `<profile>/cover_letters/cover_<company>_<role>.tex`
**Compile with:** XeLaTeX (cover.cls requires fontspec)
**Font directory:** `<profile>/cover_letters/OpenFonts/fonts/`

### Compile command

```bash
cd <profile>/cover_letters && xelatex -interaction=nonstopmode cover_<company>_<role>.tex
```
...
- Adjust closing to local convention for a non-English posting (e.g. "Med venlig hilsen," in Danish)
