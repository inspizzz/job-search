...
<!-- Shared formatting guidance. Personal statements live in the active profile. -->

## Template: LaTeX moderncv (Banking Style)

All CVs use the moderncv LaTeX package with the "banking" style and "blue" color scheme.

**Output file:** `<profile>/cv/main_<company>.tex`
**Compile with:** **lualatex** on MiKTeX/TeX Live. pdflatex often fails on modern MiKTeX installs with `fontawesome5` font-expansion errors; lualatex handles the same sources cleanly.
**Master reference:** `<profile>/cv/main_example.tex` (comprehensive CV with all competencies, experience, and achievements - use as source when building targeted CVs)

### Compile command

```bash
cd <profile>/cv && lualatex -interaction=nonstopmode main_<company>.tex
```
...
% Default banking leaves these black, which clashes with the blue accent scheme.
% moderncv v2.3.1 does NOT predefine \firstnamestyle/\lastnamestyle, so guard
% with \providecommand* before \renewcommand* or the build errors "undefined".
...
\usepackage{import}
% moderncv loads hyperref itself at end of preamble; a second \usepackage{hyperref}
% causes "Option clash for package hyperref". Queue \hypersetup via \AtEndPreamble
% so it runs AFTER moderncv's own hyperref load.
...
The three `\renewcommand*` lines in the preamble are required on lualatex+MiKTeX/TeX Live. Without them the firstname, lastname, and section headings render in black even though `\moderncvcolor{blue}` is set, which looks inconsistent with the rest of the blue accent scheme (links, bullet markers, contact icons). The override forces all three to use `color1` (moderncv's accent colour, which becomes blue under `\moderncvcolor{blue}`). Both names render bold; if you prefer the firstname in regular weight, change the firstnamestyle override from `\bfseries` to `\mdseries`. Don't drop the override - on most modern installs the defaults render visibly wrong.

**Portability note (moderncv v2.3.1, verified on this machine):** v2.3.1 does not predefine `\firstnamestyle`/`\lastnamestyle`, so a bare `\renewcommand*` on them errors with "Command undefined". Precede them with `\providecommand*{\firstnamestyle}[1]{}` / `\providecommand*{\lastnamestyle}[1]{}` (as shown above) so the template compiles across moderncv versions. `\sectionstyle` is defined in v2.3.1 and needs no guard.

**hyperref:** Do NOT add `\usepackage{hyperref}` - moderncv loads hyperref itself at the end of the preamble, and a second load throws "Option clash for package hyperref". Put PDF/link settings inside `\AtEndPreamble{\hypersetup{...}}` so they apply after moderncv's own load.
...
**Read 2-3 personal statement variants from
`<profile>/profile/03-cv-statements.md`, then tailor one to the posting.**
Keep candidate-specific examples in that file. This shared guide supplies structure
only: professional focus, strongest source-backed evidence, and contribution to the
target role. Verify existing statements against the current candidate profile
before reuse; historical wording is not evidence that a claim is still current.
