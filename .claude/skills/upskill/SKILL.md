...
- **`/upskill`** — aggregate mode: analyses all jobs in `<profile>/job_search_tracker.csv`
...
4. Check `<profile>/upskill/` for the most recent aggregate report file (`report-YYYY-MM-DD.md`) — if one exists, note its date and load it for the diff in Step 8.
...
3. Read `<profile>/profile/01-candidate-profile.md` for the candidate's current skills.
...
Remove any skill from the list that is already present in the candidate profile (`<profile>/profile/01-candidate-profile.md`). Be generous — if the profile mentions a skill in any form (e.g. "Python" covers "Python scripting"), remove it.
...
- **Aggregate:** `<profile>/upskill/report-YYYY-MM-DD.md`
- **Targeted:** `<profile>/upskill/report-YYYY-MM-DD-<company-slug>-<role-slug>.md`
  - Slugify: lowercase, spaces → hyphens, strip special characters
  - Example: `<profile>/upskill/report-2026-04-20-guardsix-senior-ai-engineer.md`
...
After saving, print:
> "Report saved to `<profile>/upskill/<filename>.md`. Review it anytime to track your learning progress."
...
3. **Targeted mode ignores the tracker.** In targeted mode, analyse only the fetched posting. Do not load or reference `<profile>/job_search_tracker.csv`.
