...
### 1a. <profile>/documents/cv/
Read all files in `<profile>/documents/cv/`. Extract:
...
### 1b. <profile>/documents/linkedin/
Read all files in `<profile>/documents/linkedin/`. Extract:
...
### 1c. <profile>/documents/diplomas/
Read all files in `<profile>/documents/diplomas/`. Extract:
- All course/module names listed on transcripts
- Thesis title and subject area
- Any specialisation or track name

### 1d. <profile>/documents/references/
Read all files in `<profile>/documents/references/`. Extract:
...
### 1e. GitHub Profile
Look up the GitHub username from `<profile>/profile/01-candidate-profile.md`. If a GitHub URL or username is present:
...
Check `<profile>/profile/01-candidate-profile.md` for any other URLs (portfolio site, personal website, Kaggle, Google Scholar, ResearchGate, publication links). For each:
- Fetch the page
...
Remove anything already present in `<profile>/profile/01-candidate-profile.md` or `<profile>/profile/02-behavioral-profile.md`.
...
### Additions to `<profile>/profile/01-candidate-profile.md`
...
### Additions to `<profile>/profile/02-behavioral-profile.md`
...
### Added to <profile>/profile/01-candidate-profile.md
[List each competency added, with source]

### Added to <profile>/profile/02-behavioral-profile.md
