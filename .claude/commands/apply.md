...
Using the framework from `<profile>/profile/04-job-evaluation.md`, evaluate the job posting against the candidate's profile. If the salary lookup tool is configured, run:
...
You already have `<profile>/profile/01-candidate-profile.md` and `<profile>/profile/04-job-evaluation.md` in context from Step 1. **Do not re-read them.**
...
- `<profile>/profile/writing-preferences.md` (if present; personal observations supplement shared rules)
...
### Cover Letter (`<profile>/cover_letters/cover_<company>_<role>.tex`)
- **Match the language of the job posting** (a non-English posting gets a cover letter in that language)
...
<CV_DRAFT file="<profile>/cv/main_<COMPANY>.tex">
<INSERT_CV_DRAFT_HERE>
</CV_DRAFT>

<COVER_LETTER_DRAFT file="<profile>/cover_letters/cover_<COMPANY>_<ROLE>.tex">
...
```json
{
  "file": "<profile>/cv/main_<COMPANY>.tex" | "<profile>/cover_letters/cover_<COMPANY>_<ROLE>.tex",
...
- **Compile from the directory holding the `.tex`**, as shown. `cover.cls` resolves
  `Path = OpenFonts/fonts/raleway/` relative to the compile working directory, so running
  xelatex from the repo root silently loses the Raleway font.
...
**CV (`<profile>/cv/main_<company>.pdf`):**
...
**Cover letter (`<profile>/cover_letters/cover_<company>_<role>.pdf`):**
- [ ] Exactly 1 page
...
- `<profile>/cv/main_<company>.tex`
- `<profile>/cover_letters/cover_<company>_<role>.tex`
