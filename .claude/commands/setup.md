...
Otherwise, before greeting the user, scan the `<profile>/documents/` folder. Use Glob with `<profile>/documents/**/*` and count files per subfolder (`cv/`, `linkedin/`, `diplomas/`, `references/`, `applications/`).

Then welcome the user with a single message that lists three paths. The wording changes based on what was found.

**If `<profile>/documents/` has files** in one or more subfolders, lead with Path A:
...
>
> I see files in your `<profile>/documents/` folder: [list per subfolder, e.g. "2 in cv/, 1 in linkedin/, 3 in references/"]. Three ways to start:
>
> **Path A: Read my documents folder** (recommended for what you have) - I'll read everything in `<profile>/documents/`, cross-reference for consistency, and build your profile from real source materials. Idempotent and safe to re-run as you add more documents.
>
...
**If `<profile>/documents/` is empty or missing**, surface Path A as a "do this if you have materials" option:
...
> Three ways to start:
>
> **Path A: Documents folder** (best signal if you have several materials) - Drop your CV / LinkedIn export / diplomas / reference letters in the `<profile>/documents/` folder, then say "go". I'll read everything and build your profile from it. See `<profile>/documents/README.md` for the folder layout.
>
> **Path B: Single CV import** - Paste or @-mention a single CV/resume here. I'll extract it and ask follow-up questions for what's missing.
>
...
Wait for the user's choice. If they pick A but the folder is still empty, tell them what to add (point at `<profile>/documents/README.md`) and stop.

---

## Path A: Documents Folder

Reads structured documents in `<profile>/documents/`, cross-references them for consistency, and merges extracted data into the seven profile skill files. Read-before-write and idempotent: changes already present will not be proposed again.
...
If every subfolder is empty, stop and tell the user to populate the folder. Point at `<profile>/documents/README.md` for the layout.
...
- `<profile>/profile/07-interview-prep.md`

Also read `<profile>/profile/03-cv-statements.md` and, if it exists,
`<profile>/profile/writing-preferences.md`. The shared `03`, `05`, and `06` guides
are formatting references only; all personal observations are written within the
active profile.
...
**Additive changes:** entirely new content not in the skill file in any form. Examples: a certification not in `<profile>/profile/01-candidate-profile.md`, a new endorsement skill, a referee not yet listed, a new behavioral quote from a reference letter, a new award.
...
- **`<profile>/profile/02-behavioral-profile.md`:** Source is LinkedIn About + recommendation letters. Extract recurring themes, adjectives, phrases about how the candidate works. Add only to "Strongest Behavioral Traits", "How [Candidate] Works Best", or "Management Style Preferences" sections. Do not overwrite existing scored assessments. Always label inferred additions: *[Inferred from LinkedIn About / Reference letter - review before relying on this]*
- **`<profile>/profile/writing-preferences.md`:** Source is `cover_letter.tex` files. Extract recurring patterns. Add as observations under "## Patterns Observed in Past Applications". Do not modify shared writing rules. Only add if 2+ cover letters show a genuine pattern.
- **`<profile>/profile/04-job-evaluation.md`:** Source is `job_posting.md` + `outcome.md` pairs. If an application reached interview or offer: note role type and sector as a confirmed strong-fit signal. If 2+ applications repeat a no-response or rejection pattern: note it. Add findings under "## Calibration from Past Applications". Do not modify the existing scoring framework.
- **`<profile>/profile/03-cv-statements.md`:** Source is `cv_draft.tex` files. Extract any personal statement not already recorded. Label with: *[Used for: <company>_<role>]*. Do not write it into the shared CV guide.
- **`<profile>/profile/writing-preferences.md`:** Source is `cover_letter.tex` files. Extract opening patterns, bullet structures, and closing formulations. Add only distinct patterns under "## Cover Letter Patterns". Do not write them into the shared letter guide.
- **`<profile>/profile/07-interview-prep.md`:** Source is CV bullets, LinkedIn descriptions, reference letter quotes. Identify achievements not yet covered by an existing STAR example. Do NOT draft full STAR examples. Add stubs under "## STAR Candidates (Complete Manually)":
...
**Current in <profile>/profile/01-candidate-profile.md:**
...
Then proceed to Step 3 to populate the non-skill files (`<profile>/profile/00-summary.md`, `<profile>/cv/main_example.tex`, `<profile>/profile/search-queries.md`). Step 3 will detect that the seven skill files are already populated and skip those substeps.
...
- **Job portals:** "The framework ships CLIs for LinkedIn, Indeed UK, Adzuna (GB), RemoteOK and WeWorkRemotely. Are these the right ones for you, or do you use other sites?" Note: Indeed and Adzuna are hard-configured for the UK; if the user targets another market, point them at `/add-portal` to add a local board, or at LinkedIn plus Google `site:` searches.
...
### 1. Update `<profile>/profile/00-summary.md`
Replace all `[PLACEHOLDER]` tokens with the user's actual information. This is the at-a-glance
candidate summary (identity, education, experience, skills, sectors, deal-breakers) loaded at the
start of every session for this profile.

Also update the local `<profile>/PROFILE.md` (display name, market, status).
Profiles are discovered from local manifests; never add a personal roster or any
candidate details to tracked `CLAUDE.md`, `AGENTS.md`, or `profiles/README.md`.
Before writing personal data, verify the active profile directory is ignored by
Git. The tracked `profiles/example/` scaffold must never be populated.

### 2. Populate `<profile>/profile/01-candidate-profile.md` *(Path B and C; skip if Path A populated it)*
...
### 3. Populate `<profile>/profile/02-behavioral-profile.md` *(Path B and C; skip if Path A populated it)*
Write the behavioral profile based on assessment results or synthesized answers.

### 4. Update `<profile>/profile/04-job-evaluation.md` *(Path B and C; skip if Path A populated it)*
...
### 5. Update `<profile>/profile/03-cv-statements.md` *(Path B and C; skip if Path A populated it)*
Add 2-3 role-specific profile statement / elevator-pitch templates based on their background,
following the guidance in `.claude/skills/job-application-assistant/05-cv-templates.md`.

**Do not edit `05-cv-templates.md` itself** — it is shared across profiles and holds only the
LaTeX skeleton and formatting rules.

### 6. Update `<profile>/profile/07-interview-prep.md` *(Path B and C; skip if Path A populated it)*
...
### 7. Update `<profile>/cv/main_example.tex`
...
### 8. Generate `<profile>/profile/search-queries.md`
...
>
> - `<profile>/profile/00-summary.md` - Your candidate summary
> - `<profile>/profile/01-candidate-profile.md` - Structured profile
> - `<profile>/profile/02-behavioral-profile.md` - Behavioral assessment
> - `<profile>/profile/04-job-evaluation.md` - Personalized evaluation framework
> - `<profile>/profile/03-cv-statements.md` - CV profile statements for your role types
> - `<profile>/profile/07-interview-prep.md` - STAR examples from your experience
> - `<profile>/cv/main_example.tex` - Your LaTeX CV template
> - `<profile>/profile/search-queries.md` - Job search queries for `/scrape`
>
> **Try it out:**
...
If Path A left any STAR stubs in `<profile>/profile/07-interview-prep.md`, also note:

> Path A flagged [N] STAR candidate stubs in `<profile>/profile/07-interview-prep.md` that need your situation/task/action/result details before you use them in interviews.

---

## Design Principles

- Three onboarding paths converge on the same skill files. Step 0 picks the right path based on what's in `<profile>/documents/`. Steps 3 and 4 are shared.
