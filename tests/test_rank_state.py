...
class RankStateCase(unittest.TestCase):
    def setUp(self):
        ...
        self.tmp = root / "profiles/fixture"
        self.tmp.mkdir(parents=True)
        (self.tmp / "PROFILE.md").write_text("Synthetic test profile")
        (root / ".gitignore").write_text("/profiles/*\n")
        subprocess.run(["git", "init", "-q", str(root)], check=True)
        (root / "tools").mkdir()
        for name in ("rank_state.py", "profile_paths.py"):
            shutil.copy2(REPO / "tools" / name, root / "tools" / name)
        self.tool = root / "tools/rank_state.py"
    ...
    def run_tool(self, *args, expect=0):
        proc = subprocess.run(
            [sys.executable, str(self.tool), *args, "--profile", "fixture", "--state", str(self.state), "--today", TODAY],
            capture_output=True,
            text=True,
            encoding="utf-8",
        )
...
class Candidates(RankStateCase):
    ...
    def test_missing_state_file_exits_nonzero(self):
        proc = subprocess.run(
            [sys.executable, str(self.tool), "candidates", "--profile", "fixture", "--state", str(self.tmp / "nope.json"),
...
class Apply(RankStateCase):
    def test_retrieval_failure_never_marks_an_entry_expired(self):
        self.write_state({"a": entry()})
        out = self.run_tool("apply", "--results", self.results([{"key": "a", "status": "retrieval_failed"}]))
        self.assertEqual(out["expired"], [])
        self.assertEqual(self.read_state()["a"]["status"], "new")

    def test_invalid_gate_rejects_entire_batch(self):
        self.write_state({"a": entry(), "b": entry()})
        before = self.state.read_bytes()
        scores = dict.fromkeys(("technical", "experience", "behavioral", "career"), 80)
        out = self.run_tool("apply", "--results", self.results([
            {"key": "a", "status": "scored", "scores": scores},
            {"key": "b", "status": "scored", "scores": scores, "eligibility_gate": "maybe"},
        ]), expect=1)
        self.assertFalse(out["written"])
        self.assertEqual(self.state.read_bytes(), before)

    def test_confirmed_ineligibility_vetoes_high_score(self):
        self.write_state({"a": entry()})
        out = self.run_tool("apply", "--results", self.results([{
            "key": "a", "status": "scored", "scores": dict.fromkeys(
                ("technical", "experience", "behavioral", "career"), 95),
            "eligibility_gate": "FAIL", "eligibility_note": "Confirmed role requirement conflict",
        }]))
        self.assertEqual(out["ranked"], [])
        self.assertEqual(out["vetoed"][0]["key"], "a")

    def test_results_outside_selected_profile_are_rejected(self):
        self.write_state({"a": entry()})
        other = self.tmp.parent / "someone-else.json"
        other.write_text("[]")
        proc = subprocess.run([sys.executable, str(self.tool), "apply", "--profile", "fixture",
                               "--results", str(other)], capture_output=True, text=True)
        self.assertNotEqual(proc.returncode, 0)
        self.assertIn("inside the selected profile", proc.stderr)
        ...
        for dimension in dimensions:
            for value in invalid:
                with self.subTest(dimension=dimension, value=value):
                    ...
                    self.assertEqual(out["ranked"], [])
                    self.assertFalse(out["written"])
                    self.assertNotIn("rank_score", self.read_state()["b"])
